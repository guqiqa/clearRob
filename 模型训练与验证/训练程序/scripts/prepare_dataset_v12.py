#!/usr/bin/env python3
"""V12: garbages_annotated (3,817 manual bboxes) + trainval_voc (7,778 VOC) + 25.6% full-image supplement"""
...
def quality_report(pd):
    ...
    print(f"\n{'='*60}")
    print(f"  V12 PRE-TRAINING QUALITY REPORT")
    print(f"{'='*60}")
    ...
    for img_file in (pd / 'train' / 'images').iterdir():
        ...
        elif s.startswith('tvl_'): src = 'trainval_voc'
        elif s.startswith('ga_'): src = 'garbages_annotated'
        else: src = 'unknown'
        src_count[src] += 1
    ...
    verdict = 'PASS — below safety line' if full_img/tr_total<0.36 else 'WARN — above safety line'
...
def main():
    ...
    sources = load_yaml(PROJECT_ROOT / "configs/source_datasets_v12.yaml")
    pd = PROJECT_ROOT.parent / "数据集" / "processed_v12"
    ...
    print("=" * 60)
    print("  V12: garbages_annotated + trainval_voc + 25.6% full-image")
    print("=" * 60)
    total_imgs = 0
