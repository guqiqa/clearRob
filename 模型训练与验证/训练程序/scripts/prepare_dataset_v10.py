...
def conv_folder(ds, m_rules, pd):
    """folder_class with box_margin support"""
    p = make_abs(ds["path"])
    if not p.exists(): return 0, {}
    mi = pd / "merged" / "images"; ml = pd / "merged" / "labels"
    stats = {"t": 0, "k": 0, "d": 0, "pc": defaultdict(int)}; imgs = 0
    ncls = ds.get("samples_per_class", None)
    margin = ds.get("box_margin", 1.0)  # default: full image
...
def quality_report(pd):
    ...
    print(f"\n{'='*60}")
    print(f"  V10 PRE-TRAINING QUALITY REPORT")
    print(f"{'='*60}")
...
def main():
    ...
    sources = load_yaml(PROJECT_ROOT / "configs/source_datasets_v10.yaml")
    pd = PROJECT_ROOT.parent / "数据集" / "processed_v10"
    ...
    print("=" * 60)
    print("  V10: Real Detection + 35% Full-Image with Margin")
    print("=" * 60)
    total_imgs = 0
