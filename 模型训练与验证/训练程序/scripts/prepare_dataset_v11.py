#!/usr/bin/env python3
"""V10: real detection + 35% full-image with margin + pre-training quality report"""
...
def conv_labelme_folder(ds, m_rules, pd):
    ...
    for cd in sorted(p.iterdir()):
        ...
        # Use folder name as fallback class hint
        ...
        for jf in sorted(cd.glob("*.json")):
            ...
            for s in shapes:
                ...
                pts = s.get("points", [])
                if len(pts) < 2: stats["d"] += 1; continue
                # Polygon → bbox: min/max of all points
                ...
                lines.append(f"{COARSE_IDS[target]} {cx:.6f} {cy:.6f} {nw:.6f} {nh:.6f}\n")
                stats["k"] += 1; stats["pc"][target] += 1
            wm(ip, mi, ml, f"ga_{cd.name}_{jf.stem}", lines); imgs += 1
    return imgs, stats
...
def conv_folder(ds, m_rules, pd):
    ...
    for cd in sorted(p.iterdir()):
        ...
        for img_file in all_imgs:
            ...
            wm(img_file, mi, ml, f"gb_{cd.name}_{img_file.stem}", [line])
...
def conv_csv(ds, m_rules, pd):
    ...
    for ip, target in all_rows:
        ...
        line = f"{cid} 0.5 0.5 {margin:.6f} {margin:.6f}\n"
        wm(ip, mi, ml, f"leaf_{ip.stem}", [line])
...
def conv_voc(ds, m_rules, pd):
    ...
    # Auto-detect VOC2007 layout (Annotations/ + JPEGImages/) vs flat layout
    ann_dir = p / "Annotations" if (p / "Annotations").exists() else p
    img_dir = p / "JPEGImages" if (p / "JPEGImages").exists() else p
    ...
    for xf in sorted(os.listdir(str(ann_dir))):
        ...
        for obj in root.findall("object"):
            ...
            stats["k"] += 1; stats["pc"][lbl] += 1
        prefix = "tvl" if (p / "Annotations").exists() else "pdl"
...
def quality_report(pd):
    """生成完整的预训练数据集质检报告"""
    names = ['recyclable','kitchen_waste','hazardous','other_waste','green_waste','road_obstacle','pedestrian_pet','stain']
    print(f"\n{'='*60}")
    print(f"  V11 PRE-TRAINING QUALITY REPORT")
    print(f"{'='*60}")

    # A. 类别分布
    class_counts = defaultdict(int)
    for split in ['train','val','test']:
        for lf in (pd / split / 'labels').iterdir():
            with open(lf) as f:
                for line in f:
                    parts = line.strip().split()
                    if parts: class_counts[(split, int(parts[0]))] += 1
    print(f"\n  A. CLASS DISTRIBUTION")
    print(f"  {'Class':<20} {'train':>8} {'val':>8} {'test':>8} {'total':>8} {'status':>10}")
    ...
    for cid in range(8):
        ...
        st = 'OK' if tr>=500 else ('WEAK' if tr>0 else 'MISSING')
        print(f"  {names[cid]:<20} {tr:>8} {vl:>8} {te:>8} {total:>8} {st:>10}")
    tr_total = sum(class_counts.get(('train',c),0) for c in range(8))
    print(f"  {'TOTAL':<20} {tr_total:>8}")
    ...
    for cid in range(8):
        ...
        print(f"  {names[cid]:<20} median={med:.1f}%  IQR=[{p25:.1f}%,{p75:.1f}%]")

    # E. 数据源组成
    src_count = Counter()
    for img_file in (pd / 'train' / 'images').iterdir():
        s = img_file.stem
        if s.startswith('taco_'): src = 'TACO'
        elif s.startswith('dat_'): src = 'datas'
        elif s.startswith('street_'): src = 'street_obstacle'
        elif s.startswith('pdl_'): src = 'puddle_voc'
        elif s.startswith('puddle_large_'): src = 'puddle_large'
        elif s.startswith('self_'): src = 'self_collected'
        elif s.startswith('gb_'): src = 'garbages'
        elif s.startswith('leaf_'): src = 'leaf_classify'
        else: src = 'unknown'
        src_count[src] += 1
    print(f"\n  E. DATA SOURCE COMPOSITION (train)")
    for src, cnt in src_count.most_common():
        pct = cnt/sum(src_count.values())*100
        print(f"  {src:<25} {cnt:>6} ({pct:.1f}%)")

    # F. 交叉校验
    tr_sizes = set()
    for img_file in random.sample(list((pd / 'train' / 'images').iterdir()), min(50, len(list((pd / 'train' / 'images').iterdir())))):
        try:
    ...
    if vl_dir.exists():
        for img_file in random.sample(list(vl_dir.iterdir()), min(50, len(list(vl_dir.iterdir())))):
            try:
                from PIL import Image
                img = Image.open(img_file)
                vl_sizes.add(img.size)
            except: pass
    ...
    print(f"  SUMMARY")
    print(f"{'='*60}")
    print(f"  Full-img ratio: {full_img}/{tr_total} = {full_img/tr_total*100:.1f}%")
    print(f"  V1 safety line: 36%")
    print(f"  Verdict: {'PASS — below safety line' if full_img/tr_total<0.36 else 'WARN — above safety line'}")

def main():
    global COARSE_IDS
    mapping = load_yaml(PROJECT_ROOT / "configs/label_mapping_v9.yaml")
    COARSE_IDS = {v: k for k, v in mapping["target_classes"].items()}
    sources = load_yaml(PROJECT_ROOT / "configs/source_datasets_v11.yaml")
    pd = PROJECT_ROOT.parent / "数据集" / "processed_v11"
    ...
    print("=" * 60)
    print("  V11: Real Detection Only — garbages replaced by trainval_voc")
    print("=" * 60)
    total_imgs = 0
