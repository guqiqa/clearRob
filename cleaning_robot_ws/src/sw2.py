"""Switch test — monitors ALL 16 SBUS channels, prints changes immediately.
Run: python3 /tmp/sw2.py
Then operate each switch in turn and watch the output.
"""
...
print("ALL 16 CHANNELS — operate switches now (90s)")
print()
    ...
    while len(data)>=SBUS_LEN:
        ...
        for i in range(16):
            if abs(chs[i]-prev[i])>30:
                t=time.time()-t0
                print("%5.1fs  CH%2d: %4d -> %4d"%(t,i+1,prev[i],chs[i]))
                prev[i]=chs[i]
os.close(fd)
print("DONE")
