...
# Set back to blocking mode after open
...
print("= SBUS Channel Monitor =")
print("Operate each stick/switch — changes >20 will show")
print()
...
while time.time()-t0<120:
    ...
    while len(data)>=SBUS_LEN:
        ...
        for ch in range(16):
            ...
            chs[ch]=v
        for i in range(8):
            if abs(chs[i]-prev[i])>20:
                t=time.time()-t0
                print("%6.1fs  CH%d: %4d -> %4d"%(t,i+1,prev[i],chs[i]))
                prev[i]=chs[i]
os.close(fd)
print("DONE")
